...
/*****************************************************************************/
void EstimateHandler::calibrateGain(map<ChipId, vector<SlimMeasurement>> &hits,
                                    map<uint32_t, float> thickness,
                                    char *fileName, map<ChipId, float> &gain) {
  ...
  for (map<ChipId, vector<SlimMeasurement>>::iterator key = hits.begin();
       key != hits.end(); key++) {
    if (n++ % (hits.size() / 40) == 0)
      cerr << ".";
    ...
    if (key->second.size() >= 10) //
      if (key->first.first != 0) {
        ...
        float previousGain =
            (gain.count(key->first) > 0 ? gain[key->first] : 1);

        fileGain << " " << hex << key->first.first << " " << dec
                 << int(key->first.second) << " " << dec << key->second.size()
...
bool EstimateHandler::allSaturated(const vector<Measurement> &meas) {
  ...
  for (vector<Measurement>::const_iterator m = meas.begin(); m != meas.end();
       m++)
    if (!m->isSaturated)
      allSaturated = false;
...
double EstimateHandler::estimate_(vector<Measurement> &meas,
  ...
  if (allSaturated(meas))
    chi2 = theFitters.minimizeAllSaturated(value, nStep);
  else
    chi2 = theFitters.newtonMethodEpsilon(value, nStep);
...
double EstimateHandler::estimate(vector<Measurement> &meas,
  ...
  bool tryToRemove = (n >= 3 && (chi2 > 1.3 * n + 4 * sqrt(1.3 * n)));
  ...
  if (tryToRemove) {
    ...
    for (vector<Measurement>::iterator m = meas_.begin(); m != meas_.end();
         m++) {
