...
EnergyLossProducer::EnergyLossProducer(const edm::ParameterSet &ps) : pset(ps) {
  ...
  cerr << "\033[22;31m" << "Setting up products.." << "\033[22;0m" << endl;
...
/*****************************************************************************/
void EnergyLossProducer::beginRun(edm::Run const &run,
  ...
  cerr << "\033[22;31m" << "Starting HadronClusters.." << "\033[22;0m" << endl;
  theClusters = new HadronClusters(es, pset);

  // read fit results
  cerr << "\033[22;31m" << "Reading fit amplitudes.." << "\033[22;0m" << endl;
  readFitResults();
}

/*****************************************************************************/
pair<vector<float>, vector<float>>
EnergyLossProducer::getProbabilities(const TTrack &track,
                                     const pair<double, double> &epsilon) {
  pair<vector<float>, vector<float>> res(vector<float>(3, 0.),
                                         vector<float>(3, 999.));

  if (track.eta > etaMin && track.eta < etaMax && track.pt > ptMin &&
      track.pt < ptMax) {
    //
    ...
    for (int k = 0; k < K; k++) {
      ...
      {
        res.first[k] =
            amp[key] * exp(-0.5 * q * q); // division by sig not needed
        res.second[k] = q;
      }
    }
...
void EnergyLossProducer::produce(edm::Event &ev, const edm::EventSetup &es) {
  ...
  const vector<reco::Track> &trackCollection = *(trackHandle.product());
  ...
  for (vector<Trajectory>::const_iterator traje = trajeCollection.begin();
       traje != trajeCollection.end(); traje++, j++) {
    TTrack track;
    ...
    pair<double, double> epsilon = theDataHandler->processTrack(
        track, estimatePix, estimateStr, estimateAll);
...
#ifdef DebugOutput
    double p = trackCollection[j].pt() * cosh(trackCollection[j].eta());
    ...
    file << " " << p << " " << estimateAll.back().dEdx()     // epsilon
